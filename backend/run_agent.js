    ...
    try {
      ...
      if (result.email) {
        ...
        if (result.email.status === 'SENT') {
          console.log('✅ Email sent to:', result.email.to);
          console.log('📧 Subject:', result.email.subject);
          console.log('🆔 Message ID:', result.email.messageId);
        } else if (result.email.authRequired) {
          console.log('🔐 Connect:', result.email.authUrl);
        }
      }
      
      if (result.invoice) {
        console.log('\n=== INVOICE ===');
        if (result.invoice.status === 'CREATED') {
          console.log('✅ Invoice created:', result.invoice.invoiceNumber);
          console.log('👤 Client:', result.invoice.clientName);
          console.log('💰 Amount: $' + result.invoice.amount);
          console.log('📄 File:', result.invoice.filename);
          console.log('📂 Path:', result.invoice.filePath);
        } else if (result.invoice.error) {
          console.log('❌ Error:', result.invoice.error);
        }
      }
    ...
    try {
      ...
      if (result.email) {
        ...
        if (result.email.status === 'SENT') {
          ...
          console.log('🔐 Gmail needs to be connected');
          console.log('Visit:', result.email.authUrl);
        } else if (result.email.error) {
          console.log('❌ Error:', result.email.error);
        }
      }
      
      // Show invoice action if any
      if (result.invoice) {
        console.log('\n=== INVOICE ACTION ===');
        if (result.invoice.status === 'CREATED') {
          ...
          console.log('💰 Amount: $' + result.invoice.amount);
          console.log('📁 File:', result.invoice.filename);
          console.log('📂 Location:', result.invoice.filePath);
          console.log('📅 Due Date:', result.invoice.dueDate);
        } else if (result.invoice.error) {
          console.log('❌ Invoice Error:', result.invoice.error);
        }
      }
